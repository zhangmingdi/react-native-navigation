...
BottomTabs provide flat navigation and access, to up to five, equally important top-level root destinations. While any type of layout can be displayed in a tab, typically, [Stacks]() are used since they allow for vertical navigation withing a tab.
...
3. Each stack declares `bottomTab` options to configure the tab's text, icon, color etc.
...
* `afterInitialTab` - After the initial tab is mounted, other tabs are mounted.
