...
As mobile application become bigger and more complex, they usually end up containing multiple primary high level destinations which are logically independent of one another. The `BottomTabs` layout is used when an app contains three to five top-level destinations which should be accessible from anywhere in the app.
...
Our app now has two tabs at the bottom of the screen and our users can easily navigate between them.

<img width="40%" src={useBaseUrl('img/stack4.png')} />

## Replacing the root

Up until now, we've discussed how to navigate within the same layout structure. We'll now learn how to replace the entire layout structure to display a new Root. A real life example for this use case would be, for example, switching from a login screen to the app's main root. Replacing the root fits this use case since we'd like to discard the previous root (login root) entirely when switching to the main root.

To demonstrate this, we'll make the following changes to our code:

1. Add a login screen with a login button.
2. When the button is clicked we will switch to our main root.
...
Now that our initial root is the Login root, we're facing a new problem - how should we show the Login root only if the user is not logged in? Since out initial root is determined in the `registerAppLaunchedListener` callback, we'll check if the user is logged in there and set the appropriate root accordingly.
