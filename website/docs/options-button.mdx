...
Button icon. If the button is pushed to the overflow menu, the button [text](#text-string) is used instead.

TODO - the anchor link above is broken
...
TODO - links above broken, anchor link below also broken

### `iconInsets`

[IconInsets](iconInsets-options) are applied to the icon to translate its original position on the screen.
