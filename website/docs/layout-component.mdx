...
TODO - just a thought, ignore me if this is irrelevant:
Since UX stands for User Experience and since delays increase user wait times (right?), which is
one of the aspects of UX, wouldn't then introducing delays actually harm UX, not improve it? Again, my knowledge here is VERY limited, so feel free to ignore me if this makes no sense.
