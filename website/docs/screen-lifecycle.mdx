...
When a modal is displayed, depending on the [modalPresentationStyle](options-root#modalpresentationstyle), content behind it might be detached from hierarchy. This affects the visibility events which are emitted to the content behind the modal.
